...
def color_thresh(img, low_thresh, high_thresh):
...
def perception_step(Rover):
    ...
    ground_map, obs_map, rocks_map = identify_ground_and_other(warped, mask)
    ...
    Rover.vision_image[:,:,2] = ground_map * 255
    Rover.vision_image[:,:,0] = obs_map * 255

    xpix, ypix = rover_coords(ground_map)
    ...
    pitch = (Rover.pitch-360.0) if (Rover.pitch > 180.0) else Rover.pitch
    roll = (Rover.roll - 180.0) if (Rover.roll > 180.0) else Rover.roll
    ...
    if rocks_map.any():
        ...
        Rover.rock_dist   = np.mean(rock_dist)
    else:
